...
class SparseConvNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = InstanceConv(cfgs.num_output_channels_depth, 80, kernel_size=3, stride=1, padding=1)
        self.seq1 = nn.Sequential(
            nn.BatchNorm2d(80),
            nn.ReLU(inplace=True),
        )

        # self.conv2 = InstanceConv(80, 80, kernel_size=3, stride=1, padding=1)
        # self.seq2 = nn.Sequential(
        #     nn.BatchNorm2d(80),
        #     nn.ReLU(inplace=True),
        # )

        self.depth_pred = InstanceConv(80, cfgs.in_channels_d, kernel_size=1)
...
if __name__ == "__main__":
    ...
    image = torch.randn(1, 80, 480, 640)
    mask = torch.randn(1, 480, 640)
    model = SparseConvNet()
    x = model(image, mask.float())
