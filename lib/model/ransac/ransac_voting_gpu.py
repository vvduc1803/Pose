import torch
import lib.model.ransac.ransac_python as ransac_voting
import numpy as np
...
def ransac_voting_layer(mask, vertex, round_hyp_num, inlier_thresh=0.999, confidence=0.99, max_iter=20,
    ...
    for bi in range(b):
        ...
        while True:
            ...
            cur_inlier = ransac_voting.voting_for_hypothesis(direct, coords, cur_hyp_pts, inlier_thresh)  # [hn,vn,tn]
        ...
        all_inlier = ransac_voting.voting_for_hypothesis(direct, coords, all_win_pts, inlier_thresh)  # [1,vn,tn]
...
def estimate_voting_distribution_with_mean(mask, vertex, mean, round_hyp_num=256, min_hyp_num=4096, topk=128, inlier_thresh=0.99, min_num=5, max_num=30000, output_hyp=False):
    ...
    for bi in range(b):
        ...
        for round_idx in range(int(round_num)):
            ...
            inlier = ransac_voting.voting_for_hypothesis(direct, coords, hyp_pts, inlier_thresh)  # [hn,vn,tn]
