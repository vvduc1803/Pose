...
def spacing_increasing_discretization(depth_interval, num_intervals, shift=1.0):
    """
    Discretize a depth interval using the Spacing-Increasing Discretization strategy.

    Parameters:
    - depth_interval: Tuple of (alpha, beta) representing the depth range.
    - num_intervals: Number of intervals to discretize into.
    - shift: Optional shift applied to alpha and beta.

    Returns:
    - thresholds: List of discretization thresholds.
    """
    alpha, beta = depth_interval
    alpha_star = alpha + shift
    beta_star = beta + shift

    thresholds = np.exp(
        np.log(alpha_star) + np.log(beta_star / alpha_star) * np.arange(num_intervals + 1) / num_intervals)

    return thresholds


# Convert continuous depth values to discrete labels
def discretize_depth(depth_values, thresholds):
    """
    Assign discrete labels to depth values based on thresholds.

    Parameters:
    - depth_values: Array of continuous depth values.
    - thresholds: List of discretization thresholds.

    Returns:
    - discrete_labels: Array of discrete labels corresponding to depth_values.
    """
    discrete_labels = np.searchsorted(thresholds, depth_values, side='right') - 1
    discrete_labels = np.clip(discrete_labels, 0, len(thresholds) - 2)  # Ensure labels are within bounds
