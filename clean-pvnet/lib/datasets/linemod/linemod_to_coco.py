import os
from lib.utils2.linemod.opengl_renderer import OpenGLRenderer
import numpy as np
...
import cv2
import json
from lib.utils2.base_utils import read_pickle
from lib.utils2.linemod.linemod_config import linemod_cls_names, linemod_K, blender_K
...
def record_real_ann(model_meta, img_id, ann_id, images, annotations):
    ...
    inds = np.loadtxt(os.path.join(data_root, cls, split + '.txt'), np.str_)
    inds = [int(os.path.basename(ind).replace('.jpg', '')) for ind in inds]
...
def _linemod_to_coco(cls, split):
    data_root = '/home/ana/Study/Pose/clean-pvnet/data/linemod'
...
if __name__ == '__main__':
    ...
    _linemod_to_coco(["cat", "can"], 'train')
