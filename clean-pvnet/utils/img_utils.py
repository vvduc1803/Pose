...
def unnormalize_img(img, mean, std, in_gpu=True):
    ...
    mean = torch.tensor(mean, dtype=torch.float32).reshape(1, 1, 3)
    std = torch.tensor(std, dtype=torch.float32).reshape(1, 1, 3)

    # Unnormalize: reverse the normalization formula (assuming input is in [0, 1] or [-1, 1])
    img = img * std + mean  # Element-wise multiplication and addition

    # Clamp to ensure values are in valid range (0 to 255) after unnormalization
    img = torch.clamp(img * 255.0, 0, 255)  # Scale to [0, 255] and clamp
