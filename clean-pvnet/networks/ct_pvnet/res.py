...
from lib.utils2 import net_utils, data_utils
from lib.utils2.tless import tless_config, tless_pvnet_utils
import torch
import numpy as np
import cv2
...
class Network(nn.Module):
    def __init__(self):
        ...
        det_meta = cfgs.det_meta
        self.dla_ct = get_pose_net(det_meta.num_layers, det_meta.heads)
        self.pvnet = get_res_pvnet(cfgs.heads['vote_dim'], cfgs.heads['seg_dim'])

        net_utils.load_network(self.dla_ct, cfgs.det_model)
        net_utils.load_network(self.pvnet, cfgs.kpt_model)
